...
int between_two_date (struct pod *first_pod, struct date *start_day, struct date *finish_day);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {      
    while (first_pod != NULL){
        int check = between_two_date (first_pod, start_day, finish_day);
        if (check == 1){
...
int between_two_date (struct pod *first_pod, struct date *start_day, struct date *finish_day){
    int s_y = start_day -> year;
    int f_y = finish_day -> year;
    int s_m = start_day -> month;
    int f_m = finish_day -> month;  
    int s_d = start_day -> day;
    int f_d = finish_day -> day;
    int s_d_count = 0;
    int f_d_count = 0;
    if (first_pod -> when -> year > s_y){
        s_d_count = 1;
    } else if (first_pod -> when -> year == s_y && first_pod -> when -> month > s_m){
        s_d_count = 1;
    } else if (first_pod -> when -> year == s_y && first_pod -> when -> month == s_m && first_pod -> when -> day >= s_d){
        s_d_count = 1;
    } 
    if (first_pod -> when -> year < f_y){
        f_d_count = 1;
    } else if (first_pod -> when -> year == f_y && first_pod -> when -> month < f_m){
        f_d_count = 1;
    } else if (first_pod -> when -> year == f_y && first_pod -> when -> month == f_m && first_pod -> when -> day <= f_d){
        f_d_count = 1;
    }  
    return s_d_count * f_d_count;
}      
